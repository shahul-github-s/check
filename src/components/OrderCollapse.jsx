...
const OrderCollapse = ({ order, active, setActive, index, withSubmenu }) => {
  ...
  // Default values if order.user is not defined
  const userFirstName = order.user?.firstName || "N/A";
  const userLastName = order.user?.lastName || "N/A";
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <button
            className={`btn-collapse ${active === order.id ? "active" : ""}`}
          ...
          {withSubmenu && (
            <IconButton className="dark:bg-widget dark:border-widget" />
          )}
        </div>
      </div>
        ...
        <div className="border rounded-[10px] mt-4">
          <table className="w-full border-collapse">
            <tbody>
              <tr>
                <td
                  className="font-semibold text-header p-2 border-b"
                  colSpan="2"
                >
                  Customer
                </td>
              ...
              <tr>
                ...
                <td className="p-2 border-b">${order.amount || "N/A"}</td>
              </tr>
              <tr>
                <td className="font-semibold text-header p-2 border-b border-r">
                  Method:
                </td>
                <td className="p-2 border-b">{order.method || "N/A"}</td>
              </tr>
              <tr>
                <td className="font-semibold text-header p-2 border-b border-r">
                  Date:
                ...
                <td className="p-2 border-b">
                  {dayjs(order.date).format("DD.MM.YY") || "N/A"}
                </td>
              </tr>
              <tr>
                ...
                <td
                  className="p-2"
                  style={{
                    color: `var(--${status?.color || "default-color"})`,
                  }}
                >
