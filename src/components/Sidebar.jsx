    ...
    <span className="nav-icon relative w-[18px]">
      ...
      {route.notifications && (
        <span className="badge">
          <span className="-mt-[1px]">{route.notifications}</span>
        </span>
      )}
    </span>
  );
  ...
  return (
    <SwipeableDrawer
      ...
      sx={{
        ...
        "& .MuiModal-backdrop": {
          backgroundColor:
            theme === "light" ? "transparent" : "rgba(0, 0, 0, .8)",
        },
      }}
    >
      <div className="flex items-center justify-between">
        <Logo />
        <button
          className="text-[24px] text-white xs:hidden"
                  ...
                  <div>
                    <div
                      className={`nav-item w-full flex items-center ${
                        activeCollapse === route.name ? "active" : ""
                      }`}
                    >
                      <Icon route={route} />
                      <span
                      ...
                      <button
                        className={`ml-auto ${
                          activeCollapse === route.name ? "expanded" : ""
                        }`}
                        aria-label={
                          activeCollapse === route.name ? "Collapse" : "Expand"
                        }
                        onClick={() => handleCollapse(route.name)}
                      >
                    ...
                    <Collapse in={activeCollapse === route.name} timeout={300}>
      ...
      <div className="mb-10 mt-10">
        ...
        <div className="flex flex-col items-start gap-8">
          {ROUTES.secondary.map((route) => (
            <NavLink key={route.name} to={route.path} className="nav-item">
              <span className="nav-icon relative w-[18px]">
                <i className={`icon icon-${route.icon}-regular`} />
                <i className={`icon icon--active icon-${route.icon}-solid`} />
              </span>
