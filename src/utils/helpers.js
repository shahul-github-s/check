 ...
 * @param prefix - prefix to be added to the formatted number (defaults to ₹)
 * @returns {*|string}
 */
export const numFormatter = (num, fractionDigits = 0, prefix = "₹") => {
  ...
  if (num >= 1000 && num < 1000000) {
    const roundedNum = (num / 1000).toFixed(fractionDigits);
    const formattedNum = parseFloat(roundedNum).toLocaleString(
      undefined,
      options
    );
    return `${prefix}${formattedNum}K`;
  } else if (num >= 1000000) {
    const roundedNum = (num / 1000000).toFixed(fractionDigits);
    const formattedNum = parseFloat(roundedNum).toLocaleString(
      undefined,
      options
    );
    ...
    const formattedNum = num.toLocaleString(undefined, options);
